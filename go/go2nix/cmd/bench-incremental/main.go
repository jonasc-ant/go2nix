...
var touchTemplates = map[string]string{
	// `var` for private — value lives in inittask, never enters PKGDEF.
	"private": "var _benchTouch = uint64(%d) %s\n",
	// `const` for exported — value IS in PKGDEF (importers inline it),
	// so rotating it changes the .x export-data file and iface cutoff
	// stays cold for downstream consumers. `var BenchTouch` would
	// declare a public variable but its value would NOT enter PKGDEF
	// (only the type signature does), so downstream wouldn't see any
	// difference and the cascade would be the same as `private`.
	"exported": "const BenchTouch = uint64(%d) %s\n",
}
